...
# ── Cloudinary config (reads from env vars set in .env) ──────────────────────
cloudinary.config(
...
def create_item():
    """
    Admin-only. Accepts multipart/form-data so images can be uploaded at the same time.
    Fields: name, description, category, price_kes, stock
...
def upload_images(item_id):
    """Upload additional images to an existing item (admin only)."""
    from models.item import Item, ItemImage
        ...
        if file and file.filename:
            try:
                result = cloudinary.uploader.upload(
                    file,
                    folder="zora/items",
                    public_id=f"item_{item_id}_{existing_count + idx}",
                    overwrite=True,
                    resource_type="image",
                )
                img = ItemImage(
...
def update_item(item_id):
    ...
    data = request.get_json(silent=True) or {}
    for field in ("name", "description", "category", "stock", "is_active"):
        if field in data:
